...
namespace EarFPS
{
    public class IntervalExerciseController : MonoBehaviour
    {
        public static IntervalExerciseController Instance { get; private set; }
        ...
        [SerializeField] private IntervalQuizController quiz;
        ...
        public void OnAnswer(bool correctAns, IntervalDef interval)
        {
            ...
            //Debug.LogWarning($"[GM] OnAnswer(correct={correct}) caller stack:\n{System.Environment.StackTrace}");
            attempts++;
        ...
        {
            //Debug.Log($"[GM] OnEnemyDestroyed called for {enemy?.name} (id={enemy?.GetInstanceID()})\n{System.Environment.StackTrace}");

            remainingEnemies--;

            Debug.Log("remaining enemies = " + remainingEnemies);
            Debug.Log("destroyed enemies = " + enemiesDestroyed);

            if (remainingEnemies <= 0 && !_gameOverShown)
            {
                _gameOverShown = true;
                StartCoroutine(WinFlowCo());
            }
        }
        ...
        {
            // no immediate feedback for wrong answer 
        }
        ...
        {
            // small celebratory beat so the final hit can read
            yield return new WaitForSeconds(1.0f);

            // Make sure time scale is normal for the overlay
            Time.timeScale = 1f;
